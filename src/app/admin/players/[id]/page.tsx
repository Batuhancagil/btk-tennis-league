...
export default function PlayerDetailPage() {
  ...
  const fetchMatchStats = useCallback(async () => {
    try {
      ...
      // Filter matches where the specific player participated
      const playerMatches = data.filter((match: Match) =>
        match.squads.some((squad) => squad.player.id === playerId)
      )
      ...
      playedMatches.forEach((match: Match) => {
        const playerSquad = match.squads.find((s) => s.player.id === playerId)
  ...
  const fetchInvitations = useCallback(async () => {
    try {
      ...
      const data = await res.json()
      // Filter invitations for this specific player
      // SUPERADMIN view includes player object, regular view doesn't
      const playerInvitations = data.filter((inv: any) => {
        if (inv.player) {
          // SUPERADMIN view includes player object
          return inv.player.id === playerId
        }
        // Regular view - invitations are already filtered by playerId in API
        // But we need to check if this invitation belongs to our player
        // Since we're superadmin, we get all invitations, so we need to filter
        // Check if there's a playerId field directly
        return inv.playerId === playerId
      })
      setInvitations(playerInvitations)
    } catch (error) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold text-gray-900 mb-2">Oyuncu Detay Paneli</h1>
            <p className="text-gray-600">{profile.name} - Profil bilgileri ve takımları</p>
          </div>
          <Link
            href="/admin"
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          {/* Teams Card */}
          <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-200 hover:shadow-xl transition-shadow">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Takımları</h2>
            {profile && profile.teams.length > 0 ? (
              ...
              <p className="text-gray-500 text-center py-4">Henüz takımı yok</p>
            )}
          </div>
